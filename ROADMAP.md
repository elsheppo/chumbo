...
## Commercial capability direction

Some valuable work belongs above the open-source runtime rather than inside a
versioned protocol helper:

- inspect an application's schemas and existing operations;
- recommend coherent capability and tool boundaries;
- improve tool names, descriptions, inputs, outputs, and next-action guidance;
- identify a disconnected bag of tools and propose a better agent work
  surface;
...
MCP by hand. A future product would sell acceleration, interpretation, and
quality assurance rather than reserving basic correctness for paying users.
